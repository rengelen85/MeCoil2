...
async function setCircleCenter() {
  if (!navigator.geolocation) {
    areaError = 'Geolocation not supported';
    return;
  }
...
async function _initCtfPreviewMap() {
  ...
  _ctfMap = _ctfL.map(ctfPreviewMapEl, {
    zoomControl: false,
...
function _renderCtfBases() {
  ...
  if (_ctfRedMarker) {
    _ctfRedMarker.remove();
    _ctfRedMarker = null;
  }
  if (_ctfBlueMarker) {
    _ctfBlueMarker.remove();
    _ctfBlueMarker = null;
  }
  ...
  if (blue) {
    _ctfBlueMarker = _ctfL
      .circleMarker([blue.lat, blue.lng], {
        radius: 10,
        color: '#448aff',
        fillColor: '#448aff',
        fillOpacity: 0.8,
...
$: if ($gameConfig.mode === GAME_MODES.CAPTURE_THE_FLAG && iAmHost)
  tick().then(_initCtfPreviewMap);
...
async function _initPreviewMap() {
  ...
  await import('leaflet/dist/leaflet.css');
  _previewMap = _previewL.map(previewMapEl, {
    zoomControl: false,
    attributionControl: false,
  });
  _previewL
...
function _renderPreviewArea() {
  ...
  if (area?.type === 'circle') {
    _previewAreaLayer = _previewL
      .circle([area.lat, area.lng], { radius: area.radiusM, ...STYLE })
      .addTo(_previewMap);
    try {
      _previewMap.fitBounds(_previewAreaLayer.getBounds(), {
        padding: [20, 20],
        maxZoom: 18,
      });
    } catch {}
    ...
    try {
      _previewMap.fitBounds(group.getBounds(), {
        padding: [30, 30],
        maxZoom: 18,
      });
    } catch {}
...
$: if ((areaType === 'circle' || areaType === 'polygon') && iAmHost)
  tick().then(_initPreviewMap);
...
$: countdown =
  $gameState === GAME_STATES.COUNTDOWN
    ? Math.max(0, Math.ceil(($countdownAt - Date.now()) / 1000))
    : null;
$: iAmHost = $myId === $hostId;
...
$: if ($gameState === GAME_STATES.COUNTDOWN) {
  tickInterval = setInterval(() => {
    countdown = Math.max(0, Math.ceil(($countdownAt - Date.now()) / 1000));
  }, 200);
} else {
...
async function setBase(team) {
  if (!navigator.geolocation) {
    baseSetError = 'Geolocation not supported';
    return;
  }
  settingBase = true;
  baseSetError = '';
